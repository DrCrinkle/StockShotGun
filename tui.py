...
from brokers import (
    ...
    dspacTrade, tradierGetHoldings
)

BROKERS = [
    "Robinhood", "Tradier", "TastyTrade", "Public", 
    "Firstrade", "Fennel", "Schwab", "BBAE", "DSPAC"
]
...
def run_tui():
    ...
    def create_main_menu():
        body = [urwid.Text(u'Brokers:'), urwid.Divider()]

        def toggle_buy_sell(button):
            current_order['action'] = 'sell' if current_order['action'] == 'buy' else 'buy'
        ...
        body.append(urwid.Button("Submit All Orders", on_press=lambda button: asyncio.create_task(submit_all_orders(button))))
        body.append(urwid.Button("View Holdings", on_press=lambda button: asyncio.create_task(show_holdings_screen(button))))
    ...
    def broker_toggle(checkbox, state):
        ...
        if state:
            if broker not in current_order['selected_brokers']:
                current_order['selected_brokers'].append(broker)
        else:
            if broker in current_order['selected_brokers']:
                current_order['selected_brokers'].remove(broker)
    ...
    async def show_holdings_screen(button):
        ...
        # Currently only Tradier supports holdings view
        broker = selected_brokers[0]  # Get the first selected broker
        if broker != "Tradier":
            response.set_text("Holdings view currently only supported for Tradier!")
            return
        ...
        try:
            ...
            holdings = await tradierGetHoldings(ticker_filter)
    ...
    def create_holdings_screen(holdings_data, broker_name):
        ...
        body = [
            ...
            text_widget,
            urwid.Divider(),
            urwid.AttrMap(urwid.Button("Back to Main Menu", on_press=show_main_screen), 
